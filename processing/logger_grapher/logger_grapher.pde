...
void setup()
{
  ...
  logfilename = folderName + "Seismo-" + nf(year(),4) + nf(month(),2) + nf(day(),2) + "-" + nf(hour(),2) + nf(minute(),2) + nf(second(),2) + ".log";
  startLogging();
}
...
String formatLogEntry()
{
 ...
 Second = nf(second(),2);
 String log_entry =
 Hour + ":" + Minute + ":" + Second + "," + sensors[0] + "," + sensors[1] + "," + sensors[2];
 return log_entry;
}
...
void openLogFile()
{
 ...
 logfilename = dataFolder + "Seismo-" + nf(year(),4) + nf(month(),2) + nf(day(),2) + "-" + nf(hour(),2) + nf(minute(),2) + nf(second(),2) + ".log";
 }
